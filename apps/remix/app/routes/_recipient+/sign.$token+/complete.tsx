...
import { DocumentShareButton } from "@signtusk/ui/components/document/document-share-button";
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const canSignUp =
    !isExistingUser && env("NEXT_PUBLIC_DISABLE_SIGNUP") !== "true";
...
export default function CompletedSigningPage({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "relative mt-6 flex w-full flex-col items-center justify-center",
          {
          ...
          {match({ status: signingStatus, deletedAt: document.deletedAt })
            .with({ status: "COMPLETED" }, () => (
              <p className="mt-2.5 max-w-[60ch] text-center text-sm font-medium text-muted-foreground/60 md:text-base">
                <Trans>
                  Everyone has signed! You will receive an email copy of the
                  signed document.
                </Trans>
              </p>
            ))
            .with({ status: "PROCESSING" }, () => (
              <p className="mt-2.5 max-w-[60ch] text-center text-sm font-medium text-muted-foreground/60 md:text-base">
                <Trans>
                  All recipients have signed. The document is being processed
                  and you will receive an email copy shortly.
                </Trans>
              </p>
            ))
            .with({ deletedAt: null }, () => (
              <p className="mt-2.5 max-w-[60ch] text-center text-sm font-medium text-muted-foreground/60 md:text-base">
                <Trans>
                  You will receive an email copy of the signed document once
                  everyone has signed.
                </Trans>
              </p>
            ))
            .otherwise(() => (
              <p className="mt-2.5 max-w-[60ch] text-center text-sm font-medium text-muted-foreground/60 md:text-base">
                <Trans>
                  This document has been cancelled by the owner and is no longer
                  available for others to sign.
                </Trans>
              </p>
            ))}
