...
import { getDocumentCertificateAuditLogs } from "@signtusk/lib/server-only/document/get-document-certificate-audit-logs";
 ...
 */
export default function SigningCertificate({
  loaderData,
  ...
  const getAuthenticationLevel = (recipientId: number) => {
    ...
    const actionAuthMethod =
      insertedAuditLogsWithFieldAuth.at(0)?.data?.fieldSecurity?.type;
    ...
    if (!authLevel) {
      const accessAuthMethod =
        extractedAuthMethods.derivedRecipientAccessAuth.at(0);
  ...
  const getRecipientAuditLogs = (recipientId: number) => {
    return {
      [DOCUMENT_AUDIT_LOG_TYPE.EMAIL_SENT]: auditLogs[
              ...
              {document.recipients.map((recipient, i) => {
                ...
                return (
                  <TableRow key={i} className="print:break-inside-avoid">
                    ...
                    >
                      <div className="hyphens-auto break-words font-medium">
                        {recipient.name}
                      </div>
                    ...
                    <TableCell
                      truncate={false}
                      className="w-[min-content] align-top"
                    >
                      {signature ? (
                        <>
                          <div
                      ...
                      <p className="text-muted-foreground mt-1 text-sm print:text-xs">
                        ...
                        <span className="inline-block">
                          {getDevice(
                            logs.DOCUMENT_RECIPIENT_COMPLETED[0]?.userAgent
                          )}
                        </span>
                      </p>
                    </TableCell>
