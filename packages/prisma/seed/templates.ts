...
} from '@signtusk/lib/constants/direct-templates';
import { incrementTemplateId } from '@signtusk/lib/server-only/envelope/increment-id';
import { prefixedId } from '@signtusk/lib/universal/id';
...
export const seedTemplate = async (options: SeedTemplateOptions) => {
  ...
  return await prisma.envelope.create({
    data: {
      ...
      recipients: {
        create: {
          email: 'recipient.1@signtusk.com',
          name: 'Recipient 1',
