...
import type { ResendTransportOptions } from './types/transport';
  ...
  public send(
    ...
    this._client.emails
      ...
      .then((response) => {
        if (response.error) {
          const statusCode =
            RESEND_ERROR_CODES_BY_KEY[response.error.name] ?? 500;
          throw new Error(
            `[${statusCode}]: ${response.error.name} ${response.error.message}`,
          );
        }
