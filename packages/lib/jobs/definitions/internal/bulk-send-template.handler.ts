...
const ZRecipientRowSchema = z.object({
  ...
  email: z.union([
    z
      .string()
      .email({ message: "Value must be a valid email or empty string" }),
    z
      .string()
      .max(0, { message: "Value must be a valid email or empty string" }),
  ]),
});
...
export const run = async ({
  ...
  const rows = parse<any>(csvContent, {
    columns: true,
  ...
  const requiredHeaders = recipients.map(
  ...
  for (const [rowIndex, row] of rows.entries()) {
    try {
      ...
      const envelope = await io.runTask(
        `create-document-${rowIndex}`,
        async () => {
      ...
      results.errors.push(
        `Row ${rowIndex + 1}: Was unable to be processed - ${errorMessage}`
      );
    }
  }
