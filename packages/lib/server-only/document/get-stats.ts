...
import type { PeriodSelectorValue } from '@signtusk/lib/server-only/document/find-documents';
import { prisma } from '@signtusk/prisma';
import { isExtendedDocumentStatus } from '@signtusk/prisma/guards/is-extended-document-status';
...
export const getStats = async ({
  ...
  ownerCounts.forEach((stat: any) => {
    stats[stat.status as keyof typeof stats] = stat._count._all;
  });

  notSignedCounts.forEach((stat: any) => {
    stats[ExtendedDocumentStatus.INBOX] += stat._count._all;
  });

  hasSignedCounts.forEach((stat: any) => {
...
const getTeamCounts = async (options: GetTeamCountsOption) => {
  ...
  let notSignedCountsGroupByArgs: any = null;
  let hasSignedCountsGroupByArgs: any = null;
