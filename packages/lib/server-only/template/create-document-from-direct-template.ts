...
import type { TSignFieldWithTokenMutationSchema } from "@signtusk/trpc/server/field-router/schema";
...
export const createDocumentFromDirectTemplate = async ({
  ...
  if (
    ...
    (!directTemplateEnvelope.documentMeta?.allowDictateNextSigner ||
      directTemplateEnvelope.documentMeta?.signingOrder !==
  ...
  const { branding, settings, senderEmail, emailLanguage } =
  ...
  if (
    ...
    directTemplateRecipient.role === RecipientRole.CC
  ) {
    throw new AppError(AppErrorCode.INVALID_REQUEST, {
  ...
  if (
    directTemplateEnvelope.updatedAt.getTime() !== templateUpdatedAt.getTime()
  ) {
    throw new AppError(AppErrorCode.INVALID_REQUEST, {
  ...
  const isAccessAuthValid = match(derivedRecipientAccessAuth.at(0))
    .with(
      ...
      () => user && user?.email === directRecipientEmail
    )
  ...
  const createDirectRecipientFieldArgs = await Promise.all(
    fieldsToProcess.map(async (templateField) => {
      ...
      const signatureImageAsBase64 =
        isSignatureField && isBase64 ? value : undefined;
  ...
  const directTemplateNonSignatureFields =
    createDirectRecipientFieldArgs.filter(
  ...
  const { createdEnvelope, recipientId, token } = await prisma.$transaction(
    async (tx) => {
      ...
      let nonDirectRecipientFieldsToCreate: Omit<
        Field,
        "id" | "secondaryId" | "templateId"
      >[] = [];

      Object.values(nonDirectTemplateRecipients).forEach(
        (templateRecipient) => {
          ...
          nonDirectRecipientFieldsToCreate =
            nonDirectRecipientFieldsToCreate.concat(
              templateRecipient.fields.map((field) => ({
                envelopeId: createdEnvelope.id,
                envelopeItemId:
                  oldEnvelopeItemToNewEnvelopeItemIdMap[field.envelopeItemId],
      ...
      const auditLogsToCreate: CreateDocumentAuditLogDataResponse[] = [
        ...
        ...createdDirectRecipientFields.map(
          ({ field, derivedRecipientActionAuth }) =>
            createDocumentAuditLogData({
      ...
      const translations =
        await getDocumentCreatedFromDirectTemplateTranslations(
      ...
      const emailTemplate = createElement(
        DocumentCreatedFromDirectTemplateSimple,
        {
          recipientName: directRecipientEmail,
          documentName: createdEnvelope.title,
      ...
      const [html, text] = await Promise.all([
        renderSimple(emailTemplate),
        renderSimple(emailTemplate, { plainText: true }),
      ]);
  ...
  try {
    ...
    await triggerWebhook({
      ...
      data: ZWebhookDocumentSchema.parse(
        mapEnvelopeToWebhookDocumentPayload(refetchedEnvelope)
      ),
