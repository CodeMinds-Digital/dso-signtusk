...
import { useFieldPageCoords } from '@signtusk/lib/client-only/hooks/use-field-page-coords';
  ...
  return (
    <FieldContainerPortal field={field}>
      <div
        ...
        className={cn(
          'field--FieldRootContainer field-card-container dark-mode-disabled group relative z-20 flex h-full w-full items-center rounded-[2px] bg-white/90 ring-2 ring-black/10 transition-all',
          color?.base,
          {
            'px-2': field.type !== FieldType.SIGNATURE && field.type !== FieldType.FREE_SIGNATURE,
            'justify-center': !field.inserted,
            'ring-amber-400': isValidating && isFieldUnsignedAndRequired(field),
          },
          className,
        )}
      >
