import type { DocumentData } from "@signtusk/lib/constants/prisma-enums";
import {
  DocumentDataType,
  EnvelopeType,
...
import { getPresignGetUrl } from "@signtusk/lib/universal/upload/server-actions";
...
export const downloadDocumentBetaRoute = authenticatedProcedure
  ...
  .query(async ({ input, ctx }) => {
    ...
    const documentData: DocumentData | undefined =
      envelope.envelopeItems[0]?.documentData;
    ...
    if (documentData.type !== DocumentDataType.S3_PATH) {
      throw new AppError(AppErrorCode.INVALID_REQUEST, {
        message:
          "Document is not stored in S3 and cannot be downloaded via URL.",
      });
    }
    ...
    try {
      const data =
        version === "original"
          ? documentData.initialData || documentData.data
          : documentData.data;
